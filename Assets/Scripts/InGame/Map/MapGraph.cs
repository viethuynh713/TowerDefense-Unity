                ...
                foreach (var item in linkedListArray[src])
                {
                    ...
                    for (int i = 0; i < path.Count - 1; i++)
                    {
                        ...
                        if ((node.x > 0 && item.x == node.x - 1 && item.y == node.y)
                            || (node.x < InGameService.mapWidth - 1 && item.x == node.x + 1 && item.y == node.y)
                            || (node.y > 0 && item.x == node.x && item.y == node.y - 1)
                            || (node.y < InGameService.mapHeight - 1 && item.x == node.x && item.y == node.y + 1))
                        {
                            isValid = false;
                            break;
                        }
                    ...
                    if ((item.y > 0 && !linkedListArray.TryGetValue(new Vector2Int(item.x, item.y - 1), out value))
                        || (item.y < InGameService.mapHeight - 1 && !linkedListArray.TryGetValue(new Vector2Int(item.x, item.y + 1), out value))
                        || (item.x > InGameService.columnIndexSplit + 1 && !linkedListArray.TryGetValue(new Vector2Int(item.x - 1, item.y), out value))
                        || (item.x < InGameService.mapWidth - 2 && !linkedListArray.TryGetValue(new Vector2Int(item.x + 1, item.y), out value))
                        || (item.y > 0 && item.x > InGameService.columnIndexSplit + 1 && !linkedListArray.TryGetValue(new Vector2Int(item.x - 1, item.y - 1), out value))
                        || (item.y < InGameService.mapHeight - 1 && item.x > InGameService.columnIndexSplit + 1 && !linkedListArray.TryGetValue(new Vector2Int(item.x - 1, item.y + 1), out value))
                        || (item.y > 0 && item.x < InGameService.mapWidth - 2 && !linkedListArray.TryGetValue(new Vector2Int(item.x + 1, item.y - 1), out value))
                        || (item.y < InGameService.mapHeight - 1 && item.x < InGameService.mapWidth - 2 && !linkedListArray.TryGetValue(new Vector2Int(item.x + 1, item.y + 1), out value)))
                    {
